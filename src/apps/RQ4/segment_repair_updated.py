...
def repair(current_model, pca, epoch, components, x_test, x_test_pred, x_test_truth, x_emds, x_test_trace, x_train, train_trace, top_n, random_r=False):
    ...
    with torch.no_grad():
        for i in range(len(x_test)):
            ...
            if len(real_) == 0:
                #This means that Xm is zero, this we now make Xm  from I(qi-2,xi-1)
              print("I(qi-1,xi) is zero. Using I(qi-2,xi-1).")
              pred_label = x_test_pred[i-1]
              truth_label = x_test_truth[i-1]
              x_trace = x_test_trace[i-1]             
              str_result += (str(i-1) + ':' + get_str(x_test[i-1], 0, pred_label, truth_label,x_trace ,
                                                       transition, xemd))
              str_result += '\r\n'
              train_index, test_end, prev_state, prev_label = find_key(x_trace, transition_train, truth_label)
              if train_index is None or len(train_index) == 0:
              ...
              if len(real_) == 0:
                  ...
                  print(truth_label, 'Found potential err (Extended Influence Function used here)', x_test[i-1][1][test_end],
                  'Number of train can reach the previous state:', len(real_),)
            else: 
        

             print(truth_label, 'Found potential err (Normal Influence Function used here)', x_test[i][1][test_end],
                  'Number of train can reach the previous state:', len(real_))
